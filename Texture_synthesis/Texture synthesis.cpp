...
int main()
{
	...
	for (int i = TileSize - OverLapSize; i <= Height - TileSize; i += (TileSize - OverLapSize))
		...
		{
			Mat area_1(dest, getRect(i, j, TileSize, OverLapSize));// col direction
			Mat area_2(dest, getRect(i, j, OverLapSize, TileSize));// row direction
			Mat area_0(dest, getRect(i, j, OverLapSize, OverLapSize));// little square
			double dis = 1e10;
			int x, y;
			for(int m = 0;m<=DefaultSize-TileSize;++m)
				for (int n = 0; n <= DefaultSize - TileSize; ++n)
			...
			generateMask(area_1, area_2, c_area_1, c_area_2, mask);// compute the mask
			Mat copy_d(dest, getRect(i, j, TileSize, TileSize));
			Mat copy_s(source, getRect(x, y, TileSize, TileSize));
			for(int m=0;m<TileSize;++m)
				for (int n = 0; n < TileSize; ++n)
				{
					if (mask[m][n]) // Part of the mask is updated
